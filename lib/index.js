...
export default class SurveyMonkey extends Activity {
    ...
    async requestDetails(config) {
      api = SurveyMonkeyAPI(apiKey, accessToken, { version: config.version, secure: config.secure });
      return new Promise((resolve, reject)  => {
        api.getSurveyDetails({ survey_id: config.survey_id }, function(error, data) {
          if (error)
            reject(Error(error));
          else
            resolve(data);
